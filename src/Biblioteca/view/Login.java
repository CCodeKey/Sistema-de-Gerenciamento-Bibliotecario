...
public class Login extends JFrame {
	...
	private void cadastroAction(ActionEvent e) {
		try {
			...
			if (nome.isBlank() || matricula.isBlank() || tipo.isBlank() || telefone.isBlank() || email.isBlank()
					|| cpf.isBlank() || senha.isBlank()) {
			...
			if (!tipo.equals("bibliotecário") && !tipo.equals("estagiário")) {
				JOptionPane.showMessageDialog(this, "Tipo de usuário inválido! Use: bibliotecário ou estagiário.",
			...
			if (!telefone.matches("\\d{10,11}")) {
				JOptionPane.showMessageDialog(this, "Telefone inválido! Deve conter apenas números (com DDD).", "Erro",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			JOptionPane.showMessageDialog(this, "Informações inválidas ou usuário já existe!", "Erro",
					JOptionPane.ERROR_MESSAGE);
